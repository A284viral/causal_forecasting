...
def main_function(config, simulate_change, selected_causal = None, change_percent = {}):
    ...
    for model in models:
        ...
        if simulate_change:
            sales_before = result_df[result_df["yearmonth"] >= config["test_start_date"]]['outlier_corrected_data'].sum()
            sales_after = df1[df1["yearmonth"] >= config["test_start_date"]][final_forecast_var].sum()
